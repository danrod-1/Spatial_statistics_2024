...
For CV, Matern is marginally better (we want to minimize the CV). While no model is outright wrong if we need to pick one spherical seems like the best option. Yes, Matern has a better CV however the difference is so small that it may be reasonable to prioritize increased performance as measured by loglike, AIC, BIC (also small but a bit larger) 

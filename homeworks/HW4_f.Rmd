...
```
```{r}
ggplot(result_table97, aes(x = X, y = Y)) +
  # Confidence interval as shaded area
  geom_tile(aes(fill = Predicted_Value)) +
  geom_tile(aes(fill = Lower_Bound), alpha = 0.4) +
  geom_tile(aes(fill = Upper_Bound), alpha = 0.4) +
  geom_point(data = shrimps_geor97$coords, aes(x = X, y = Y), color = "red", size = 1) +
  scale_fill_gradient(low = "lightblue", high = "darkblue") +
  labs(title = "Predicted Values with 95% Confidence Intervals", fill = "Prediction") +
  theme_minimal()
```
...
```
```{r}
ggplot(result_table06, aes(x = X, y = Y)) +
  # Confidence interval as shaded area
  geom_tile(aes(fill = Predicted_Value)) +
  geom_tile(aes(fill = Lower_Bound), alpha = 0.4) +
  geom_tile(aes(fill = Upper_Bound), alpha = 0.4) +
